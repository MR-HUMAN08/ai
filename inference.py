...
def log_step(step_label, action, reward, done, error=None):
    print(f"[STEP] step={step_label} action={action} reward={reward:.2f} done={str(done).lower()} error={error or 'null'}", flush=True)
